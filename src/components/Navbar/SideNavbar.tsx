        ...
        <ul className="bg-white h-full w-full relative flex flex-col">
            <li className="w-full border-b py-3 px-4 mb-3">
                <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild className="px-4 w-full flex justify-between">
                        ...
                        >
                            {value
                                ? hospitalList.find((framework) => framework.value === value)?.label
                                : "Select for hospitals"}
            ...
            </li>
            {
                links.map((link, i) => (
                    ...
                    <li key={i}>
                        <Link className={cn("hover:bg-secondary text-[14px] transition-all mx-4 px-3 rounded-lg border-l-4 border-transparent text-muted-foreground active:scale-95 py-2 mb-1 cursor-pointer font-medium flex gap-3 items-center", '/' + pathname.split('/').slice(1).join("/") === link.link && "bg-primary/10 text-primary border-primary")} href={link.link}>
                            {link.icon}
                            <p>{link.name}</p>
                        </Link>
                    </li>
            ...
            <li className="mt-auto">
                <Link className="hover:bg-secondary text-[14px] transition-all mx-4 px-3 rounded-lg border-l-4 border-transparent text-muted-foreground active:scale-95 py-2 mb-1 cursor-pointer font-medium flex gap-3 items-center" href="/hospital/settings">
                    <Settings size={size} />
                    <p>Settings</p>
                </Link>
            </li>
